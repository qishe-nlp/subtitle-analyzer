...
def merge_content(npfile, ppfile, vpfile):
  content = []
  nps, pps, vps = [], [], []
  ...
  with open(ppfile, newline='') as csvfile:
    reader = csv.DictReader(csvfile, delimiter='\t', dialect='excel')
    pps = [row for row in reader]   
  with open(vpfile, newline='') as csvfile:
    reader = csv.DictReader(csvfile, delimiter='\t', dialect='excel')
    vps = [row for row in reader]   
  line_num = len(nps)
  assert(line_num == len(vps) and line_num == len(pps))
  for i in range(line_num):
    np, pp, vp = nps[i], pps[i], vps[i]
    sentence, start, end = np["sentence"], np["start"], np["end"]
    assert(start == pp["start"] and pp["start"] == vp["start"])
    assert(end == pp["end"] and pp["end"] == vp["end"])
    assert(sentence == pp["sentence"] and pp["sentence"] == vp["sentence"])
    content.append({"sentence": sentence, "start": start, "end": end, "noun_phrases": json.loads(np["phrases"]), "prep_phrases": json.loads(pp["phrases"]), "verbs": json.loads(vp["vs"])})
  return content

@click.command()
@click.option("--npfile", help="Specify the reviewed noun phrase filename", prompt="noun phrase source")
@click.option("--ppfile", help="Specify the reviewed prep phrase filename", prompt="prep phrase source")
...
def gen_phrase_ass(npfile, ppfile, vpfile, lang, srtfile, assfile, google):
  content = merge_content(npfile, ppfile, vpfile)
  print(content)
  analyzer = PhraseAnalyzer(lang)
